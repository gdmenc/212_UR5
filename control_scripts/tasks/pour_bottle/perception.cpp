/**
 * perception.cpp — Bottle detection + body grasp candidates
 * Task: pour_bottle
 *
 * Detects a bottle (COCO class 39) with YOLOv8 via a RealSense D435i,
 * back-projects the bbox centre to a 3D camera-frame point, transforms
 * to task frame, and prints 24 side body grasp candidates as JSON
 * (8 angles × 3 heights: 0.04, 0.07, 0.10 m).
 *
 * Geometry mirrors control_scripts/grasps/bottle.py exactly.
 *
 * Dependencies
 * ------------
 ...
 *
 * Run:
 *   ./build/perception_bottle
 *   ./build/perception_bottle --model /path/to/yolov8n.onnx --conf 0.4
 */
...
static constexpr double BOTTLE_GRASPABLE_TOP_Z_M  = 0.130;  // graspable up to 13 cm
static constexpr double BOTTLE_PREGRASP_OFFSET     = 0.050;  // 5 cm along tool -Z
static constexpr double BOTTLE_GRASP_FORCE         = 20.0;   // N — heavier than cup/bowl
...
// Grasp heights inside the graspable cylinder region.
static const std::array<double, 3> GRASP_HEIGHTS_M = {0.04, 0.07, 0.10};
...
// ---------------------------------------------------------------------------
// Rotation: side body grasp at angle_rad
// Mirrors bottle.py::_side_body_rotation:
//   R_z(angle) * (R_y(-π/2) * R_z(π/2) * R_z(π/2))
// ---------------------------------------------------------------------------

static Eigen::Matrix3d side_body_rotation(double angle_rad) {
    // Precomputed base rotation (independent of angle):
    //   R_y(-π/2) * R_z(π/2) * R_z(π/2)
    static const Eigen::Matrix3d BASE = []() -> Eigen::Matrix3d {
        Eigen::Matrix3d a = Eigen::AngleAxisd(-M_PI / 2.0,
                                             Eigen::Vector3d::UnitY()).matrix();
        Eigen::Matrix3d b = Eigen::AngleAxisd( M_PI / 2.0,
                                             Eigen::Vector3d::UnitZ()).matrix();
        return a * b * b; // R_y(-π/2) * R_z(π/2) * R_z(π/2)
    }();
...
// bottle_base_task: position of the bottle's resting base centre in task frame.
static std::vector<GraspCandidate> bottle_body_candidates(
    const Eigen::Vector3d& bottle_base_task,
    int n_angles = N_ANGLES)
{
    std::vector<GraspCandidate> out;
    out.reserve(n_angles * (int)GRASP_HEIGHTS_M.size());

    for (double grasp_z : GRASP_HEIGHTS_M) {
        for (int i = 0; i < n_angles; i++) {
            double angle = 2.0 * M_PI * i / n_angles;

            Eigen::Vector3d body_offset(
                BOTTLE_BODY_RADIUS_M * std::cos(angle),
                BOTTLE_BODY_RADIUS_M * std::sin(angle),
                grasp_z);
            Eigen::Vector3d grasp_t = bottle_base_task + body_offset;
            Eigen::Matrix3d R       = side_body_rotation(angle);

            // Pregrasp: BOTTLE_PREGRASP_OFFSET along tool -Z (= -R.col(2))
            Eigen::Vector3d pre_t = grasp_t - BOTTLE_PREGRASP_OFFSET * R.col(2);

            std::ostringstream desc;
            desc.precision(0);
            desc << std::fixed << "bottle body grasp @ "
                 << (angle * 180.0 / M_PI) << "deg, z="
                 << (grasp_z * 100.0) << "cm";
    ...
    const std::vector<GraspCandidate>& cands)
{
    ...
    std::cout << "  \"bottle_base_task_m\": " << v3(bottle_base)<< ",\n";
    std::cout << "  \"candidates\": [\n";
    for (int i = 0; i < (int)cands.size(); i++) {
        ...
        std::cout << "      \"grasp_z_m\": "        << g.grasp_z_m    << ",\n";
...
int main(int argc, char** argv) {
    ...
    std::cerr << "[INFO] Detecting bottle (COCO " << COCO_BOTTLE << ") over "
              << N_FRAMES << " frames...\n";
    bool found = false;
    ...
    for (int fi = 0; fi < N_FRAMES; fi++) {
        ...
        auto dets = yolo_detect(net, bgr, COCO_BOTTLE, conf_threshold);
    ...
    std::cerr << "[INFO] Bottle at camera ("
    ...
    // Bottle base: detected bbox centre ≈ midpoint of the graspable body.
    // Shift down by GRASPABLE_TOP_Z / 2 to approximate the base position.
    Eigen::Vector3d bottle_base = pos_task;
    bottle_base[2] -= BOTTLE_GRASPABLE_TOP_Z_M / 2.0;

    auto cands = bottle_body_candidates(bottle_base);
    print_json(true, best_conf, pos_camera, pos_task, bottle_base, cands);
    return 0;
}
