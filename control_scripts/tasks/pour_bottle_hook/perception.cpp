/**
 * perception.cpp - Bottle detection -> task-frame base/neck for hook pouring.
 *
 * stdout JSON:
...
static constexpr double BOTTLE_BODY_RADIUS_M = 0.0365;
static constexpr double BOTTLE_TOTAL_HEIGHT_M = 0.175;
static constexpr double BOTTLE_OPENING_RADIUS_M = 0.020;
...
public:
    ...
    bool loadModel(const std::string& model_path, const std::string& classes_path) {
        ...
        num_classes = static_cast<int>(class_list.size());
    ...
    bool runFrame(const cv::Mat& bgr, float conf_thr, float& best_conf, cv::Rect& best_box) {
        if (version == YOLOVersion::UNKNOWN || bgr.empty()) return false;

        cv::Mat img = bgr.clone();
        int max_side = std::max(img.cols, img.rows);
        cv::Mat formatted = cv::Mat::zeros(max_side, max_side, CV_8UC3);
        img.copyTo(formatted(cv::Rect(0, 0, img.cols, img.rows)));

        float xf = static_cast<float>(formatted.cols) / INPUT_WIDTH;
        float yf = static_cast<float>(formatted.rows) / INPUT_HEIGHT;

        cv::Mat blob;
        cv::dnn::blobFromImage(
            formatted, blob, 1.0 / 255.0, cv::Size(INPUT_WIDTH, INPUT_HEIGHT),
        ...
        if (!outs.empty() && !outs[0].empty()) {
            if (version == YOLOVersion::YOLOV5) {
                parseV5(outs[0], xf, yf, conf_thr, boxes, confs);
            } else {
                parseV8V11(outs[0], xf, yf, conf_thr, boxes, confs);
            }
        }
    ...
    bool depthSanity(
        ...
        float min_m = 0.20f,
        float max_m = 3.0f) const
    {
        int cx = box.x + box.width / 2;
        int cy = box.y + box.height / 2;
        int hw = std::max(1, box.width / 4);
        int hh = std::max(1, box.height / 4);
        ...
        Eigen::Vector3d& out_base_task)
    {
        ...
        float pix[2] = {static_cast<float>(cx), static_cast<float>(cy)};
        float pt[3];
...
private:
    bool detectVersion() {
        ...
        cv::Mat blob;
        cv::dnn::blobFromImage(
            dummy, blob, 1.0 / 255.0, cv::Size(INPUT_WIDTH, INPUT_HEIGHT),
            cv::Scalar(), true, false);
        net.setInput(blob);
        ...
        net.forward(outs, net.getUnconnectedOutLayersNames());
        if (outs.empty()) {
            std::cerr << "[ERROR] No model output\n";
            return false;
        }

        int d1 = outs[0].size[1];
        int d2 = outs[0].size[2];
        ...
        if (d1 > 1000 && d2 < 200) {
            ...
            std::cerr << "[ERROR] Unknown output shape\n";
            return false;
        }
        return true;
    }
        ...
        std::vector<float>& confs)
    {
        ...
        for (int i = 0; i < num_detections; ++i, data += dims) {
            ...
            cv::Point cl;
            double ms;
            cv::minMaxLoc(scores, nullptr, &ms, nullptr, &cl);
            float conf = obj * static_cast<float>(ms);
            if (cl.x != COCO_BOTTLE || conf < thr) continue;

            float x = data[0], y = data[1], w = data[2], h = data[3];
            boxes.emplace_back(
                static_cast<int>((x - w / 2.0f) * xf),
                static_cast<int>((y - h / 2.0f) * yf),
    ...
    void parseV8V11(
        ...
        std::vector<float>& confs)
    {
        int nf = out.size[out.dims == 3 ? 1 : 0];
        int nd = out.size[out.dims == 3 ? 2 : 1];
        cv::Mat m2d(nf, nd, CV_32F, out.data);
        cv::Mat tr;
        cv::transpose(m2d, tr);
        float* data = reinterpret_cast<float*>(tr.data);
        ...
        for (int i = 0; i < num_detections; ++i, data += (4 + num_classes)) {
            ...
            double ms;
            cv::minMaxLoc(scores, nullptr, &ms, nullptr, &cl);
            float conf = static_cast<float>(ms);
            if (cl.x != COCO_BOTTLE || conf < thr) continue;
    ...
    const std::string& save_path)
{
    ...
    if (detected) {
        ...
        label << std::fixed;
        label.precision(2);
        label << "bottle (hook)  conf=" << conf;
        cv::putText(disp, label.str(), cv::Point(box.x, std::max(0, box.y - 8)),
                    cv::FONT_HERSHEY_SIMPLEX, 0.55, cv::Scalar(0, 255, 0), 2);

        std::ostringstream base, neck;
        base << std::fixed;
        base.precision(3);
        neck << std::fixed;
        neck.precision(3);
        base << "base_task [m]: " << base_task[0] << ", " << base_task[1] << ", " << base_task[2];
        neck << "neck_task [m]: " << neck_task[0] << ", " << neck_task[1] << ", " << neck_task[2];
        put(base.str(), 0);
        put(neck.str(), 1);
        put("BOTTLE HOOK DETECTED", 2, cv::Scalar(0, 200, 255));
    } else {
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (a == "--model" && i + 1 < argc) {
            ...
            if (!ok) {
                std::cerr << "[ERROR] --T-task-camera needs 16 floats\n";
                return 1;
            }
        }
    }

    (void)BOTTLE_BODY_RADIUS_M;
    (void)BOTTLE_OPENING_RADIUS_M;
    ...
    for (int fi = 0; fi < N_FRAMES; ++fi) {
        ...
        cv::Mat bgr(cv::Size(cf.get_width(), cf.get_height()),
                    CV_8UC3, const_cast<void*>(cf.get_data()), cv::Mat::AUTO_STEP);
    ...
    Eigen::Vector3d bottle_base_task(0.0, 0.0, 0.0);
    Eigen::Vector3d bottle_neck_task(0.0, 0.0, BOTTLE_TOTAL_HEIGHT_M);
    Eigen::Vector3d pos_cam(0.0, 0.0, 0.0);
    ...
    if (!headless && !best_frame.empty()) {
        save_annotated_frame(
            best_frame, best_box, best_conf, bottle_base_task, bottle_neck_task, found, save_image);
    }
