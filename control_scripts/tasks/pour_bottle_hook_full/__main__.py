...
from . import pour_bottle_hook as _hook_mod
...
def run(
    ...
    with default_session(left=True, right=False) as session:
        ...
        arm.gripper.open()  # pre-open so the hook is clear before the planned approach
        _confirm("Plan looks correct? Execute PICK, POUR and PLACE?", auto)

        print("\n" + "-" * 62)
        print(f"[5-6/6]  Executing pick → pour → place (motion-planned transits) ...")
        success = run_on_arm(
            session, arm, grasp, pour_pose, place_pose, CONFIG,
            bottle_base_pose=bottle_base_pose,
        )
        if not success:
