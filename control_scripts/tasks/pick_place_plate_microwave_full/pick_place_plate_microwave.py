...
-------
Standalone:
    python -m control_scripts.tasks.pick_place_plate_microwave_full [--dry]
"""
...
def _plan_sim_legs(grasp, place_pose: Pose, config: PickPlaceConfig):
    ...
    if MOTION_PLAN_PRE_PICK_APPROACH:
        ...
        try:
            ...
            print(f"  x pre-pick approach infeasible: {exc}")
            return legs
    ...
    try:
        ...
        print(f"  x post-pick carry plan infeasible: {exc}")
        return legs
    ...
    try:
        ...
        print(f"  x post-place return plan infeasible: {exc}")
        return legs
    ...
    *,
    ...
    if plate_normal_tcp is not None:
        print("  carry plate level: plate normal within "
              f"+-{np.degrees(CARRY_PLATE_LEVEL_TOLERANCE_RAD):.1f} deg of task +Z")
    ...
    print("  live TCP vs wp0: "
          ...
          f"drot={np.degrees(np.linalg.norm(tcp_rot_delta.as_rotvec())):.2f} deg")
    if plate_normal_tcp is not None:
        ...
        print(f"  live plate tilt estimate: {live_tilt:.2f} deg from task +Z")
    ...
    try:
        ...
        print(f"  x motion plan infeasible: {exc}")
    ...
    if not result.success:
        print(f"  x planned transit execution failed: {result.reason}")
        if MOTION_PLAN_AUTO_FALLBACK:
            return _run_movel_fallback(
                f"execute_plan failed: {result.reason}",
            )
        return False
    print("  v planned transit reached.")
    return True
...
def _print_plan(grasp, place_pose: Pose) -> None:
    ...
    print(f"  Grasp angle   : {np.degrees(GRASP_ANGLE_RAD):+.0f} deg  (plate-rim azimuth)")
    print(f"  Place angle   : {np.degrees(PLACE_ANGLE_RAD):+.0f} deg  (plate-rim azimuth)")
    if PICK_FROM == "microwave":
        a = APPROACH_ANGLE_RAD
        print(f"  Approach (pick) : "
              f"{('%+.0f deg (motion dir)' % np.degrees(a)) if a is not None else 'tool-axis (entry_xy_for_pose)'}")
    if PLACE_TO == "microwave":
...
def run_on_arm(
    ...
    if not pick_result.success:
        print(f"  x pick FAILED: {pick_result.reason}")
        return False
    print("  v pick succeeded.")
    ...
    if USE_MIDPOINT:
        print("  v carried through midpoint.")
    ...
    if not place_result.success:
        print(f"  x place FAILED: {place_result.reason}")
        return False
    print("  v place succeeded.")
